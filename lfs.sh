...
#echo; echo "AbdLFS: Cpioing pre-prepro"; echo
#cd $LFSBASE/pro
#find . | cpio -o -H newc | gzip > $cdir/abd-lfs-"$buildver-$buildtag".$KARCH-after-lfs-s3-ext.cpio.gz
#cd $cdir
...
echo; echo 'AbdLFS: Executing lfs-s5-strip.sh pro';echo
export LFS=$LFSBASE/pro
env -i LFS=$LFS HOME=$HOME TERM=$TERM PS1='\u:\w\$ ' ./lfs-s5-strip.sh pro

echo; echo 'AbdLFS: Executing lfs-s5-strip.sh dev';echo
export LFS=$LFSBASE/dev
env -i LFS=$LFS HOME=$HOME TERM=$TERM PS1='\u:\w\$ ' ./lfs-s5-strip.sh dev


### Packing AbdLFSRemoved for backup and diagnostics 
